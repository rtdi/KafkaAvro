...
from .schemabuilder import ValueSchema
from . import avro_datatypes
...
class FunctionDataflow(BaseModel):
    function_name: str
...
class EventSet(BaseModel):
    ...
    on_dataflows: Optional[list[FunctionDataflow]] = None
    on_schema: Optional[list[str]] = None
    ...
    def add_dataflows_event(self, function_name: str, dataflow_name: str) -> None:
        """
        This function should be called whenever a dataflow of a given name completed

        :param function_name: The function causing this to trigger
        :param dataflow_name: The dataflow causing this to trigger
        """
        if self.on_dataflow is None:
            self.on_dataflows = [FunctionDataflow(function_name=function_name, dataflow_name=dataflow_name)]
        else:
            self.on_dataflows.append(FunctionDataflow(function_name=function_name, dataflow_name=dataflow_name))
...
class TriggerSchema(ValueSchema):
    ...
    def __init__(self):
        ...
        on_dataflows = avro_datatypes.RecordSchema("on_dataflows", doc="The function should be called for all these dataflows")
        on_dataflows.add_field("function_name", avro_datatypes.ArraySchema(avro_datatypes.AvroString()), nullable=True, doc="the function name triggering this")
        on_dataflows.add_field("dataflow_name", avro_datatypes.ArraySchema(avro_datatypes.AvroString()), nullable=True, doc="the dataflow name triggering this")
        ...
        events.add_field("on_commit", commit_event, 
                       ...
                       nullable=True)
        events.add_field("on_dataflows", avro_datatypes.ArraySchema(on_dataflows),
        ...
        events.add_field("dataflows", avro_datatypes.ArraySchema(dataflows), 
                       ...
                       nullable=True)
        events.add_field("delay_seconds", avro_datatypes.AvroInt(), nullable=True, doc="if a first trigger event occurred, wait this many seconds to collect more, thus avoiding frequent triggers")
        events.add_field("idle_seconds", avro_datatypes.AvroInt(), nullable=True, doc="wait until no trigger event occurred for this many seconds and only then start, to avoid frequent triggers")
        ...
        self.add_field("endpoint", avro_datatypes.AvroString(),
                       doc="the queue name of the function", 
                       nullable=False, aliases=["queuename"])
...
class ExecutionLogSchema(ValueSchema):
    ...
    def __init__(self):
        ...
        self.add_field("completed_ts", avro_datatypes.AvroTimestampMicros(), nullable=True)
        self.add_field("queue_msg_id", avro_datatypes.AvroString(), nullable=True)
...
class ExecutionLogEntry(BaseModel):
    ...
    create_ts: Optional[datetime] = datetime.now(timezone.utc)
